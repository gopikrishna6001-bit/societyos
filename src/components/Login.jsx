...
const inp = { width: "100%", background: "#0d1117", border: "1px solid #2a2f45", borderRadius: 10, padding: "14px 16px", color: "#e2e8f0", fontSize: 28, outline: "none", boxSizing: "border-box", fontFamily: "inherit", textAlign: "center", letterSpacing: "12px" };
...
export default function Login({ onLogin }) {
  ...
  const setFirstPin = async () => {
    ...
    if (result.error) { setError(result.error); setLoading(false); return; }
    setSuccess("PIN set! Entering app...");
    setTimeout(() => onLogin({ resident: { ...sessionData.resident, pin_changed: true }, role: sessionData.role }), 1000);
    setLoading(false);
  };

  const phoneInp = { width: "100%", background: "#0d1117", border: "1px solid #2a2f45", borderRadius: 10, padding: "14px 16px", color: "#e2e8f0", fontSize: 20, outline: "none", boxSizing: "border-box", fontFamily: "inherit", textAlign: "center", letterSpacing: "2px" };
  const btn = { background: "linear-gradient(135deg,#d97706,#f59e0b)", border: "none", borderRadius: 10, padding: "14px", color: "#0d0f14", fontWeight: 700, cursor: "pointer", fontSize: 15, width: "100%" };
  const lbl = { color: "#94a3b8", fontSize: 11, fontWeight: 600, textTransform: "uppercase", marginBottom: 6, display: "block", letterSpacing: "0.5px" };
        ...
        <div style={{ background: "#161b27", border: "1px solid #2a2f45", borderRadius: 20, padding: 28 }}>
          ...
          {step === "phone" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <div>
                ...
                <p style={{ color: "#64748b", fontSize: 13, margin: 0 }}>Enter your registered phone number</p>
              </div>
              <input style={phoneInp} type="tel" value={phone}
              ...
              <button onClick={findByPhone} disabled={loading} style={{ ...btn, opacity: loading ? 0.7 : 1 }}>
                {loading ? "Checking…" : "Continue →"}
              </button>
              <div style={{ background: "#0d1117", borderRadius: 10, padding: 14, color: "#475569", fontSize: 12, lineHeight: 1.8 }}>
          ...
          {step === "select_flat" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <button onClick={() => setStep("phone")} style={{ background: "none", border: "none", color: "#64748b", cursor: "pointer", fontSize: 13, textAlign: "left" }}>← Back</button>
              <h3 style={{ color: "#e2e8f0", fontSize: 18, fontFamily: "'Playfair Display',serif", margin: 0 }}>Select Your Flat</h3>
              {multipleFlats.map(r => (
                <button key={r.id} onClick={() => { setResident(r); setStep("pin"); }}
                  ...
                  <div style={{ textAlign: "left" }}>
                    ...
                    <div style={{ color: "#fbbf24", fontSize: 12, marginTop: 2 }}>{r.block ? `Block ${r.block} · ` : ""}Flat {r.flat_number}</div>
                  </div>
          ...
          {step === "pin" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <button onClick={() => { setStep("phone"); setPin(""); setError(""); }} style={{ background: "none", border: "none", color: "#64748b", cursor: "pointer", fontSize: 13, textAlign: "left" }}>← Back</button>
              ...
              <input style={inp} type="password" value={pin}
                onChange={e => { setPin(e.target.value.replace(/\D/g, "").slice(0, 4)); setError(""); }}
                placeholder="••••" maxLength={4}
                onKeyDown={e => e.key === "Enter" && verifyPin()} autoFocus />
              {error && <div style={{ color: "#f87171", fontSize: 13, textAlign: "center" }}>{error}</div>}
              <button onClick={verifyPin} disabled={loading || pin.length < 4} style={{ ...btn, opacity: (loading || pin.length < 4) ? 0.6 : 1 }}>
          ...
          {step === "set_new_pin" && (
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              <div style={{ textAlign: "center" }}>
                ...
                <p style={{ color: "#64748b", fontSize: 13, margin: 0 }}>First login — set a personal 4-digit PIN.</p>
              </div>
              <div>
                <label style={lbl}>New PIN</label>
                <input style={inp} type="password" value={newPin}
              ...
              <div>
                ...
                <input style={inp} type="password" value={confirmPin}
              ...
              <button onClick={setFirstPin} disabled={loading || newPin.length < 4 || confirmPin.length < 4}
                style={{ ...btn, opacity: (loading || newPin.length < 4 || confirmPin.length < 4) ? 0.6 : 1 }}>
